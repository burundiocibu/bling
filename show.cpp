...
void nrf_tx(unsigned slave, uint8_t *buff, size_t len, bool updateDisplay);
...
void slider(unsigned slave, uint8_t ch, uint16_t &v, int dir)
{
   ...
   nrf_tx(slave, buff, sizeof(buff), false);
}
   ...
   if (b!=button)
   {
	 ...
	 //TODO: for now, nothing
      ...
      else if (b & lcd_plate::SELECT)
      {
         ...
         nrf_tx(effect, buff, sizeof(buff), true);
...
void Oldprocess_ui(void)
{
   ...
   if (b!=button)
   {
      ...
      else if (b & lcd_plate::SELECT)
      {
         ...
         nrf_tx(slave, buff, sizeof(buff), true);
         logfile << 1e-3*runtime.msec() << " effect " << 0 << endl;
      }
   }
}

...
const uint8_t e1_delay[] PROGMEM = {
   //           2     3     4     5     6     7     8     9
...
Effect::Effect(uint16_t slave)
{
   ...
   se_delay[1] = 0xffff;
   if (slave_id < sizeof(e1_delay))
   {
      uint8_t d = pgm_read_byte( &(e1_delay[slave_id]));
      if  (d!=0xff)
         se_delay[1] = d * 30;
   }

   se_delay[3] = 0xffff;
   if (slave_id < sizeof(e3_delay))
   {
      uint8_t d = pgm_read_byte( &(e3_delay[slave_id]));
      if (d!=0xff)
         se_delay[3] = d * 15;
   }

   se_delay[4] = 0xffff;
   if (slave_id < sizeof(e4_delay))
   {
      uint8_t d = pgm_read_byte( &(e4_delay[slave_id]));
      if (d!=0xff)
         se_delay[4] = d * 15;
   }
